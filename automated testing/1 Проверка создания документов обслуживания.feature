...
* Я заполняю первый документ
	...
	* Я выбираю дату и время проведения работ
		...
		И в поле с именем 'ВКМ_ДатаПроведенияРабот' я ввожу текст '08.07.2024'
	...
	* Я заполняю описание проблемы
		И в поле с именем 'ВКМ_ОписаниеПроблемы' я ввожу текст 'Необходимо выехать к клиенту и определить проблему'
...
* Я заполняю второй документ
	...
	* Я заполняю документ имеющимися данными	
		И из выпадающего списка с именем "ВКМ_Клиент" я выбираю точное значение 'ООО \"Кузенька\"'
		И из выпадающего списка с именем "ВКМ_Договор" я выбираю точное значение '№1 от 07.07.2024'
	...
	* Я заполняю остальные данные документа	
		И в поле с именем 'ВКМ_ДатаПроведенияРабот' я ввожу текст '08.07.2024'
		И в поле с именем 'ВКМ_ВремяНачалаРабот' я ввожу текст '13:00:00'
		И в поле с именем 'ВКМ_ВремяОкончанияРабот' я ввожу текст '15:00:00'
		И в поле с именем 'ВКМ_ОписаниеПроблемы' я ввожу текст 'Обновление 1 ЗУП'
		И я нажимаю на кнопку с именем 'ФормаЗаписать'
		И поле с именем "Номер" заполнено Тогда
		И я запоминаю значение поля с именем "Номер" как "$$НомерДок2$$"
		И я запоминаю значение поля с именем "ВКМ_Специалист" как "$$Сотрудник$$"
		И поле с именем "Дата" заполнено 
		И Я закрываю окно 'Обслуживание клиента * от *'
		Тогда открылось окно 'Обслуживание клиентов'
